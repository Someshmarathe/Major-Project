...
def archive():
    ...
    arranged_records = []
    for row in records:
        # Ensure row[1] is a string before using .find()
        if isinstance(row[1], datetime):  # ✅ Now datetime is recognized
            row_value = row[1].strftime("%Y-%m-%d %H:%M:%S")  # Convert datetime to string
        else:
            row_value = str(row[1])  # Convert other types to string

        print(f"Row[1]: {row_value}")  # Debugging

        separator = row_value.find(",")  # Now safe to use .find()
        
        if separator == -1:  # Handle cases where ',' is not found
            print("Warning: No comma found in row[1]. Skipping this record.")
            continue
        
        lat = row_value[1:separator]
        long = row_value[separator + 1:-1]
